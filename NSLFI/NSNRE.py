...
def nested_sampling(ndim, nlive, nsim, stop_criterion, samplertype, nreSettings):
    # initialisation
    np.random.seed(234)
    ...
    livepoints = [0]
    logPosteriors = posterior_3d.log_prob(observation=x_0, v=[livepoints])
    ...
    sampler = Sampler(prior=prior, logLikelihood=posterior_3d, ndim=ndim).getSampler(samplertype)
    while logIncrease > np.log(stop_criterion):
        ...
        logPosteriors[index] = float(posterior_3d.log_prob(x=x_0, v=proposal_sample))
        livepoints_birthlogL[index] = minlogLike
        # add datapoint to NRE
        store._append_new_points(v=[proposal_sample], log_w=[mre_3d.log_ratio(observation=x_0, v=[proposal_sample])])
...
nre_settings = NRE_Settings()
logZ = nested_sampling(ndim=2, nlive=1000, nsim=100, stop_criterion=1e-2,
                       samplertype="Metropolis", nreSettings=nre_settings)
print(logZ)
